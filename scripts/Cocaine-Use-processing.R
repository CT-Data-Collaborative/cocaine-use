...
##################################################################
#
# Processing Script for Cocaine Use
...
for (i in 1:length(legacy_table_dfs)) {
  ...
  descriptions <- c("Cocaine Use in the Past Year",
                    "Cocaine Use in Past Year")  #10
...
new_table_dfs <- new_table_dfs[grepl(paste(c(" 7", " 4"), collapse = "|"), new_table_dfs)]
...
test$`Cocaine Use` <- NA
test$`Cocaine Use`[grep("Cocaine", test$`Table Description`)] <- "Cocaine Use in the Past Year"

test <- test[!is.na(test$`Cocaine Use`),]
...
#assign variable
all_data_long$Variable[grep("Estimate", all_data_long$Variable)] <- "Cocaine Use"
...
all_data_final <- all_data_long %>% 
  select(`Substate Region`, Year, `Age Range`, `Cocaine Use`, `Measure Type`, Variable, Value) %>% 
  rename(Region = `Substate Region`) %>% 
  arrange(Region, Year, `Age Range`, `Cocaine Use`, Variable)
...
write.table(
  all_data_final,
  file.path(getwd(), "data", "cocaine-use-2016.csv"),
  sep = ",",
  na = "-9999",
  row.names = F
)
